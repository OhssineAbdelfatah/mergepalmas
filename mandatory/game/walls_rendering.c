		...
		double offset)
{
	while (func->i >= 0)
	{
		...
		func->walls->wall_hight = (square_len / func->adjusted_distance)
			* func->walls->distance_prj_plane;
		func->top = (((var->window_height) / 2) - (func->walls->wall_hight / 2))
			+ var->p_infos->up_down_offset;
		func->buttom = func->top + func->walls->wall_hight;
		...
		if (is_there_door(var->p_infos->rays[func->i].bonus_rays, var, func->i))
			draw_door(var, func->walls, func->i, func->j, offset);
		func->i--;
		func->j++;
	}
}
