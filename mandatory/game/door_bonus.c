...
int	calc_x_door(int hor_or_ver, t_ray_hit_door *ray, double door_hiegt,
		double img_door_w, int i, t_main_s *var)
{
	double	x_img;
	double	a;
	...
	if (hor_or_ver == 'h')
	{
		a = var->p_infos->rays[i].bonus_rays->door->x_intersection
			/ (double)door_hiegt;
		// printf("x>>> %.5f\n", ray->x_intersection);
		// printf("---------Xinter [%f]\n", a);
	...
	{
		a = var->p_infos->rays[i].bonus_rays->door->y_intersection
			/ (double)door_hiegt;
		x_img = (a)-floor(a);
		// printf("---------Yinter [%f]\n", x_img);
	}
...
void	draw_door(t_main_s *var, t_walls *walls, int i, int j, double offset)
{
	...
	while (c + (door_h - (door_h * offset)) < door_h)
	{
		...
		if (c + top >= 0 && c + top + (door_h - (door_h
					* offset)) < var->window_height) // window hieght
		{
			y_door = calc_y_door(c, door_h, var->bonus->door->hieght);
			// color = gettt_rgba( &var->text[0].pixels[(y_door
						// * var->text->width + x_img) * 4]);
			// if(x_img < var->text[0].hieght && y_door < var->text[0].width )
			if (y_door >= 0 && x_door >= 0 && y_door < var->bonus->door->hieght
				&& x_door < var->bonus->door->width)
			{
				color = var->bonus->door->pixels[y_door][x_door];
			}
			mlx_put_pixel(var->img2, j, c + top + (door_h - (door_h * offset)),
				color);
			// printf("c+ top>>>%d\n", c + top);
